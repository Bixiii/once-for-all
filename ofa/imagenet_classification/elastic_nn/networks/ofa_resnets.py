...
from ofa.imagenet_classification.networks import ResNets, SmallResNets
from ofa.utils import make_divisible, val2list, MyNetwork

__all__ = ['OFAResNets', 'OFASmallResNets']
...
class OFASmallResNets(ResNets):
    ...
    def __init__(self, n_classes=10, bn_param=(0.1, 1e-5), dropout_rate=0,
        ...
        # build input stem
        input_stem = [
            DynamicConvLayer(val2list(3), mid_input_channel, 3, stride=1, use_bn=True, act_func='relu'),
            ResidualBlock(
                DynamicConvLayer(mid_input_channel, mid_input_channel, 3, stride=1, use_bn=True, act_func='relu'),
                IdentityLayer(mid_input_channel, mid_input_channel)
            ),
        ...
        super(OFASmallResNets, self).__init__(input_stem, blocks, classifier)
    ...
    def load_state_dict(self, state_dict, **kwargs):
        ...
        for key in state_dict:
            ...
            assert new_key in model_dict, '%s' % new_key
            model_dict[new_key] = state_dict[key]
        super(OFASmallResNets, self).load_state_dict(model_dict)

    """ set, sample and get active sub-networks """

    def set_max_net(self):
        self.set_active_subnet(d=max(self.depth_list), w=len(self.width_mult_list) - 1)

    def set_active_subnet(self, d=None, w=None, **kwargs):
        base_depth_list = [1, 1, 1, 1]
        depth = val2list(d, len(base_depth_list) + 1)
        width_mult = val2list(w, len(base_depth_list) + 2)
    ...
    def get_active_subnet(self, preserve_weight=True):
        ...
        for stage_id, block_idx in enumerate(self.grouped_block_index):
            ...
            for idx in active_idx:
                ...
                input_channel = self.blocks[idx].active_out_channel
        classifier = self.classifier.get_active_subnet(input_channel, preserve_weight)
        subnet = SmallResNets(input_stem, blocks, classifier)
