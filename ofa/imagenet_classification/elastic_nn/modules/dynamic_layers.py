...
class DynamicResNetBasicBlock(MyModule):
    ...
    def __init__(self, in_channel_list, out_channel_list, kernel_size=3, stride=1, act_func='relu', downsample_mode='conv'):
        ...
        self.conv2 = nn.Sequential(OrderedDict([
            ('conv', DynamicConv2d( max(self.out_channel_list), max(self.out_channel_list))),
            ('bn', DynamicBatchNorm2d(max(self.out_channel_list))),
        ]))
    ...
    def get_active_subnet(self, in_channel, preserve_weight=True):
        ...
        # copy weight from current layer
        sub_layer.conv1.conv.weight.data.copy_(
            self.conv1.conv.get_active_filter(self.active_out_channels, in_channel).data)
        copy_bn(sub_layer.conv1.bn, self.conv1.bn.bn)

        sub_layer.conv2.conv.weight.data.copy_(
            self.conv2.conv.get_active_filter(self.active_out_channel, self.active_out_channels).data)
        copy_bn(sub_layer.conv2.bn, self.conv2.bn.bn)
    ...
    def re_organize_middle_weights(self, expand_ratio_stage=0):
        ...
        sorted_importance, sorted_idx = torch.sort(importance, dim=0, descending=True)
        self.conv1.conv.conv.weight.data = torch.index_select(self.conv1.conv.conv.weight.data, 1, sorted_idx)
        adjust_bn_according_to_idx(self.conv1.bn.bn, sorted_idx)
        self.conv1.conv.conv.weight.data = torch.index_select(self.conv1.conv.conv.weight.data, 0, sorted_idx)
