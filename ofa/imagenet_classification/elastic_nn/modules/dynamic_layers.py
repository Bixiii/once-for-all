...
class DynamicResNetBottleneckBlock(MyModule):
    ...
    def re_organize_middle_weights(self, expand_ratio_stage=0):
        # conv3 -> conv2
        importance = torch.sum(torch.abs(self.conv3.conv.conv.weight.data), dim=(0, 2, 3))  # importance for each channel
        ...
        sorted_importance, sorted_idx = torch.sort(importance, dim=0, descending=True)  # sort importance
        self.conv3.conv.conv.weight.data = torch.index_select(self.conv3.conv.conv.weight.data, 1, sorted_idx)  # reorder channels according to their importance
        adjust_bn_according_to_idx(self.conv2.bn.bn, sorted_idx)  # also reorder batch norm layer
...
class DynamicResNetBasicBlock(MyModule):
    ...
    def __init__(self, in_channel_list, out_channel_list, expand_ratio_list=0.25,
                 kernel_size=3, stride=1, act_func='relu', downsample_mode='avgpool_conv'):
        super(DynamicResNetBasicBlock, self).__init__()
        ...
        self.conv1 = nn.Sequential(OrderedDict([
            ('conv', DynamicConv2d(max(self.in_channel_list), max(out_channel_list), kernel_size, stride)),
            ('bn', DynamicBatchNorm2d(max(out_channel_list))),
            ('act', build_activation(self.act_func, inplace=True)),
        ]))

        self.conv3 = nn.Sequential(OrderedDict([
            ('conv', DynamicConv2d(max(out_channel_list), max(out_channel_list), kernel_size)),
            ('bn', DynamicBatchNorm2d(max(out_channel_list))),
        ]))
        ...
        if self.stride == 1 and self.in_channel_list == self.out_channel_list:
            ...
            self.downsample = nn.Sequential(OrderedDict([
                ('conv', DynamicConv2d(max(self.in_channel_list), max(out_channel_list), stride=stride)),
                ('bn', DynamicBatchNorm2d(max(self.out_channel_list))),
            ]))
        elif self.downsample_mode == 'avgpool_conv':
            self.downsample = nn.Sequential(OrderedDict([
