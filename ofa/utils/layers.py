...
class ResNetBasicBlock(MyModule):
    ...
    def __init__(self, in_channels, out_channels,
                 kernel_size=3, stride=1, act_func='relu', groups=1,
        ...
        # build modules
        self.conv1 = nn.Sequential(OrderedDict([
            ('conv', nn.Conv2d(self.in_channels, self.out_channels, 3, stride, 1, bias=False)),
            ('bn', nn.BatchNorm2d(self.out_channels)),
            ('act', build_activation(self.act_func, inplace=True)),
        ]))

        self.conv2 = nn.Sequential(OrderedDict([
            ('conv', nn.Conv2d(self.out_channels, self.out_channels, 3, 1, 1, bias=False)),
            ('bn', nn.BatchNorm2d(self.out_channels)),
        ]))
